...
def download(
    ...
    *,
    ...
    Cached under ``~/.dagnam/models/{version_id}/{artifact_id}.bin``.
    Verifies the artifact's checksum against the server-reported value on
    every call, including a cache hit — a model weight file is loaded
    downstream (e.g. via ``torch.load``), so a swapped or corrupted cache
    entry is a code-execution risk, not just a data-integrity nuisance.
    Concurrent downloads of the same artifact are serialized with
    ``dataset_lock`` and promoted atomically (``os.replace`` from a
    same-directory ``.partial`` temp file), so a reader never observes a
    ...
    dest_path = version_dir / f"{cache_dir_name(artifact_id)}.bin"
    ...
    if _cached():
        touch_cache(cache_dir_name(version_id), base_dir=base_dir)
        return dest_path
    ...
    with dataset_lock(f"{version_id}/{artifact_id}", base_dir=base_dir):
        ...
        touch_cache(cache_dir_name(version_id), base_dir=base_dir)
        try:
            evict_lru_locked(base_dir=base_dir)
