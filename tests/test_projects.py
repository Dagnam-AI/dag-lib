...
class TestReadOperations:
    def test_list_passes_filters(self):
        ...
        projects.list(
            page=2, limit=10, framework="pytorch", status="active",
            visibility="public", tags=["nlp", "cv"], search="foo",
            sort_by="created_at", order="asc", client=c,
        )
    ...
    def test_list_omits_none_filters(self):
...
class TestWriteOperations:
    ...
    def test_duplicate_with_title(self):
    ...
    def test_duplicate_without_title(self):
        c = _client(duplicate_project=MagicMock(return_value={"id": "p2"}))
        projects.duplicate("p1", client=c)
        c.duplicate_project.assert_called_once_with("p1", None)

    def test_save_architecture_delegates(self):
        c = _client(save_project_architecture=MagicMock(return_value={"ok": True}))
        projects.save_architecture("p1", {"nodes": []}, {"layers": []}, commit_message="v1", client=c)
...
class TestDAGImport:
    def test_import_dag_builds_payload(self):
    ...
    def test_import_dag_existing_delegates(self):
...
class TestBulkAndDatasets:
    def test_bulk_delete_delegates(self):
    ...
    def test_link_dataset_delegates(self):
        c = _client(link_project_dataset=MagicMock(return_value={"ok": True}))
        projects.link_dataset("p1", "d1", "training", client=c)
        c.link_project_dataset.assert_called_once_with("p1", {"dataset_id": "d1", "role": "training"})

    def test_get_datasets_delegates(self):
    ...
    def test_unlink_dataset_delegates(self):
...
class TestErrorPropagation:
    def test_get_propagates_project_not_found(self):
        c = _client()
