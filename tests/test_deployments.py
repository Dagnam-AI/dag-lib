...
class TestReadDelegation:
    def test_list_passes_filters(self):
        ...
        deployments.list(
            page=2, limit=50, status="running", platform="fastapi",
            project_id="p1", search="foo", client=client,
        )
        client.list_deployments.assert_called_once_with(
            page=2, limit=50, status_filter="running", platform="fastapi",
            project_id="p1", search="foo",
        )
    ...
    def test_logs_forwards_all_filters(self):
        ...
        deployments.logs(
            "dep-1", level="error", search="boom", start_time="2026-01-01",
            end_time="2026-01-02", page=3, limit=25, client=client,
        )
        client.get_deployment_logs.assert_called_once_with(
            "dep-1", level="error", search="boom",
...
class TestLifecycleLRO:
    def test_create_returns_lro_with_initial_payload(self):
        ...
        op = deployments.create(
            name="d", project_id="p", checkpoint_path="/ckpt",
            platform="fastapi", deployment_type="text", instance_type="t3.medium",
            client=client,
        )
    ...
    def test_create_includes_optional_fields_when_set(self):
        ...
        deployments.create(
            name="d", project_id="p", checkpoint_path="/ckpt",
            platform="fastapi", deployment_type="text", instance_type="t3.medium",
            auto_scaling_enabled=True, min_instances=1, max_instances=5,
            region="us-east-1", config={"k": "v"},
            client=client,
        )
        sent = client.create_deployment.call_args.args[0]
    ...
    def test_rollback_returns_lro(self):
        ...
        client.rollback_deployment.assert_called_once_with(
            "dep-1", checkpoint_path="/ckpts/v2.pt"
        )
        assert isinstance(op, LongRunningOperation)
...
class TestClientErrorMapping:
    ...
    def test_get_maps_404(self):
        with patch("dagnam._core.client.requests.request", return_value=_mock_response(404)):
            with pytest.raises(DeploymentNotFoundError):
                self._client().get_deployment("missing")

    def test_create_maps_401(self):
        with patch("dagnam._core.client.requests.request", return_value=_mock_response(401)):
            with pytest.raises(AuthError):
                self._client().create_deployment({"name": "x"})

    def test_create_maps_422(self):
        with patch(
            "dagnam._core.client.requests.request",
    ...
    def test_list_success_returns_dict(self):
        ...
        with patch(
            "dagnam._core.client.requests.request", return_value=_mock_response(200, body)
        ):
