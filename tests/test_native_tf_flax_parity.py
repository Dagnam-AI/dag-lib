...
def test_native_tf_dataset_val_split_is_distinct(ds_factory):
    ...
    train = ds._native_tensorflow_dataset(
        split="train", batch_size=5, shuffle=False, val_ratio=0.1, seed=0,
    )
    val = ds._native_tensorflow_dataset(
        split="val", batch_size=5, shuffle=False, val_ratio=0.1, seed=0,
    )
...
def test_native_tf_dataset_train_honors_shuffle_seed(ds_factory):
    ...
    ds = ds_factory(_native_train_tf=native_train)
    a = [int(v.numpy()) for batch in ds._native_tensorflow_dataset(
        split="train", batch_size=10, shuffle=True, val_ratio=0.0, seed=123
    ) for v in batch[1]]
    b = [int(v.numpy()) for batch in ds._native_tensorflow_dataset(
        split="train", batch_size=10, shuffle=True, val_ratio=0.0, seed=123
    ) for v in batch[1]]
...
def test_native_flax_dataset_reshapes_batches_and_splits_val(ds_factory):
    ...
    train = ds._native_flax_dataset(
        split="train", batch_size=10, shuffle=False, val_ratio=0.1, seed=0,
    )
    val = ds._native_flax_dataset(
        split="val", batch_size=10, shuffle=False, val_ratio=0.1, seed=0,
    )
...
def test_resolve_system_dataset_flax_encodes_text_without_image_scaling(monkeypatch, tmp_path):
    ...
    from dagnam.data.loaders.system_loader import resolve_system_dataset_flax

    meta = dict(_META)
    meta["name"] = "imdb"
    meta["dataset_type"] = "text"
    monkeypatch.setattr(
        "dagnam.data.loaders.system_loader._SYSTEM_CACHE_ROOT",
        tmp_path,
    )
