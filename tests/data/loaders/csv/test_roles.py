...
from dagnam.data.loaders.csv import _FEATURE_ROLES, _TARGET_ROLES, _split_by_roles
...
_any_role_st = st.sampled_from(sorted(_FEATURE_ROLES) + sorted(_TARGET_ROLES) + ["ignore"])
...
def _make_df(column_names: list[str], n_rows: int = 5) -> pd.DataFrame:
    ...
    return pd.DataFrame({col: range(n_rows) for col in column_names})
