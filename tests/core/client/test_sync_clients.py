...
def test_parse_filename_quoted() -> None:
    assert _parse_filename('attachment; filename="cool.csv"') == "cool.csv"


def test_parse_filename_unquoted() -> None:
    assert _parse_filename("attachment; filename=cool.csv") == "cool.csv"


def test_parse_filename_default_when_none() -> None:
    assert _parse_filename(None) == "data"


def test_parse_filename_default_when_no_filename_param() -> None:
    assert _parse_filename("inline") == "data"
...
def test_is_success_response_from_status_code() -> None:
    class _R:
        status_code = 204
        ok = False

    assert _is_success_response(_R())


def test_is_success_response_falls_back_to_ok_attr() -> None:
    class _R:
        status_code = None
        ok = True
...
def test_safe_error_body_delegates_to_common(client) -> None:
    class _R:
        headers: ClassVar[dict[str, str]] = {"Content-Type": "text/plain"}
        content = b"err"
        text = "err"

    assert _safe_error_body(_R()) == "err"


# ---------------------------------------------------------------- hub client


def test_list_hub_models(client, rmock):
...
def test_hub_timeout_wrapped(client, monkeypatch):
    def _boom(*_a, **_kw):
...
def test_duplicate_project_with_and_without_title(client, rmock):
...
def test_save_architecture(client, rmock):
...
def test_link_unlink_dataset(client, rmock):
...
def test_projects_timeout_wrapped(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_list_deployments_all_params(client, rmock):
...
def test_list_deployments_minimal(client, rmock):
    rmock.get(f"{API}/api/v1/deployments", json={"items": []})
    client.list_deployments()
    # Only page+limit
...
def test_scale_deployment(client, rmock):
...
def test_rollback_deployment(client, rmock):
...
def test_get_deployment_metrics(client, rmock):
...
def test_get_deployment_logs_all_params(client, rmock):
...
def test_get_deployment_health_full(client, rmock):
...
def test_open_deployment_stream_sets_headers(client, rmock):
...
def test_open_deployment_stream_without_cursor(client, rmock):
...
def test_open_deployment_stream_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_open_deployment_stream_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_deployments_connection_error_wrapped(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_deployments_timeout_wrapped(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_deployments_text_response(client, rmock):
...
def test_list_datasets_with_search(client, rmock):
...
def test_list_datasets_no_search(client, rmock):
...
def test_list_datasets_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_list_datasets_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_get_dataset_meta_with_version(client, rmock):
...
def test_get_dataset_meta_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_get_dataset_meta_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_list_system_datasets(client, rmock):
    rmock.get(f"{API}/api/v1/datasets/system", json=[{"id": "iris"}])
    assert client.list_system_datasets() == [{"id": "iris"}]


def test_list_system_datasets_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_list_system_datasets_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_get_system_dataset_meta(client, rmock):
...
def test_get_system_dataset_meta_no_version(client, rmock):
    rmock.get(f"{API}/api/v1/datasets/system/iris", json={"id": "iris"})
    client.get_system_dataset_meta("iris")
    assert rmock.last_request.qs == {}


def test_get_system_dataset_meta_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_get_system_dataset_meta_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_download_system_dataset_writes_file(client, rmock, tmp_path):
    rmock.get(
...
def test_upload_dataset_streams_file(client, rmock, tmp_path):
    f = tmp_path / "data.csv"
    f.write_text("a,b\n1,2")
...
def test_upload_dataset_connection_error(client, monkeypatch, tmp_path):
    f = tmp_path / "data.csv"
    f.write_text("x")
...
def test_upload_dataset_timeout(client, monkeypatch, tmp_path):
    f = tmp_path / "data.csv"
    f.write_text("x")
...
def test_upload_dataset_from_url(client, rmock):
...
def test_upload_dataset_from_url_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_upload_dataset_from_url_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_get_dataset_task_status(client, rmock):
    rmock.get(f"{API}/api/v1/datasets/tasks/t1", json={"status": "done"})
    assert client.get_dataset_task_status("t1") == {"status": "done"}


def test_get_dataset_task_status_connection_error(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_get_dataset_task_status_timeout(client, monkeypatch):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_download_dataset_full(client, rmock, tmp_path):
    rmock.get(
...
def test_download_dataset_resume_206(client, rmock, tmp_path):
...
def test_download_dataset_resume_falls_back_to_full(client, rmock, tmp_path):
    part = tmp_path / "ds.bin.part"
...
def test_download_dataset_resume_disabled_cleans_partial(client, rmock, tmp_path):
    part = tmp_path / "ds.bin.part"
...
def test_download_dataset_with_presigned_url(client, tmp_path):
...
def test_download_dataset_connection_error(client, monkeypatch, tmp_path):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_download_dataset_timeout(client, monkeypatch, tmp_path):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
...
def test_download_dataset_404(client, rmock, tmp_path):
    rmock.get(f"{API}/api/v1/datasets/missing/download", status_code=404)
    with pytest.raises(DatasetNotFoundError):
        client.download_dataset("missing", tmp_path)


def test_download_system_dataset_connection_error(client, monkeypatch, tmp_path):
    def _boom(*_a, **_kw):
        raise requests.ConnectionError("nope")
...
def test_download_system_dataset_timeout(client, monkeypatch, tmp_path):
    def _boom(*_a, **_kw):
        raise requests.Timeout("slow")
