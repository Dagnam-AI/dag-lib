...
def test_load_config_returns_empty_on_malformed_json(isolated_config):
    (isolated_config / "config.json").write_text("not json {")
    assert config_mod.load_config() == {}


def test_load_config_returns_empty_on_oserror(isolated_config, monkeypatch):
...
def test_get_config_value_default(isolated_config):
