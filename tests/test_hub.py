...
class TestDiscovery:
    def test_search_delegates_all_filters(self):
        ...
        out = hub.search(
            search="llama", task_type="text-generation", framework="pytorch",
            license="mit", tags=["nlp"], is_official=True, is_verified=False,
            sort_by="recent", page=2, limit=10, client=c,
        )
...
class TestModelCRUD:
    ...
    def test_create_builds_payload(self):
        ...
        hub.create(
            name="my-model", description="desc", task_type="text-generation",
            framework="pytorch", tags=["nlp"], client=c,
        )
    ...
    def test_create_omits_none_optionals(self):
        c = _client(hub_create=MagicMock(return_value={"id": "m1"}))
        hub.create(
            name="m", description="d", task_type="t", framework="f", client=c,
        )
        payload = c.hub_create.call_args.args[0]
