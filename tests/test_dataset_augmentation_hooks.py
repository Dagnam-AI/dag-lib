...
def test_image_folder_loader_accepts_custom_transform(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "dagnam.data.loaders.image_folder_loader.create_pytorch_loader",
        fake_create_loader,
    )
...
def test_to_arrays_and_to_pytorch_loader_produce_same_split(tmp_path: Path):
    ...
    features_arr, labels_arr = dataset.to_arrays(
        split="train", val_ratio=0, test_ratio=0, seed=42
    )
...
def test_tensorflow_dataset_accepts_sample_and_batch_map_fns(
    ...
    monkeypatch.setattr(
        "dagnam.data.loaders.tf_loader.create_tensorflow_dataset",
        fake_create_dataset,
    )
...
def test_flax_dataset_accepts_sample_and_batch_transform_fns(
    ...
    monkeypatch.setattr(
        "dagnam.data.loaders.flax_loader.create_flax_dataset",
        fake_create_dataset,
    )
