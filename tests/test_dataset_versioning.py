...
class TestClientVersionQuery:
    ...
    def test_get_dataset_meta_sends_version_query(self):
        ...
        with patch("dagnam._core.client.requests.get", return_value=resp) as mock_get:
...
class TestVersionedCacheKey:
    ...
    def test_load_dataset_uses_versioned_cache_key(self, tmp_path: Path):
        ...
        # Write metadata
        import json
        (versioned_dir / "meta.json").write_text(
            json.dumps(meta), encoding="utf-8"
        )

        with (
            patch("dagnam.get_api_key", return_value="key"),
            patch("dagnam.get_api_url", return_value="https://api.test"),
            patch.object(DagnamClient, "get_dataset_meta", return_value=meta),
            patch.object(DagnamClient, "download_dataset", return_value=versioned_dir / "data.csv") as mock_download,
        ):
    ...
    def test_load_dataset_passes_version_and_filename_to_download(self, tmp_path: Path):
        ...
        with (
            ...
            patch("dagnam.get_api_url", return_value="https://api.test"),
            patch.object(DagnamClient, "get_dataset_meta", return_value=meta),
            patch.object(DagnamClient, "download_dataset", return_value=downloaded) as mock_download,
        ):
